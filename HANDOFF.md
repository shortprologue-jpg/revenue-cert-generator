...
0. ~~⭐웹 배포(2단계)~~ ✅ **완료·라이브(2026-07-21)** — Streamlit Cloud + Supabase.
   ...
   - ⚠️ **keepalive 만 대기 중**: GitHub Actions 가 **"Repository access blocked"** 로 막힘 — 저장소를 오늘 **public 전환 + 신규활동 많아** GitHub 이 이 저장소만 일시 제한(계정·이메일 정상, **발라내기(private)는 세션23부터 keepalive 정상 작동 중** — 앞서 "발라내기가 keepalive 놓쳤다"고 한 건 오조사였음, 정정).
     ...
     - **대응 순서**: ① 저장소 Settings>Actions>General = "Allow all actions" 정상 확인함(저장소 설정 문제 아님 = 계정 레벨 차단). ② ✅ **GitHub Support 티켓 제출 완료(2026-07-21, Type=Actions 워크플로 문제, repo URL·"account verified, restore Actions" 기재).** → **이메일(shortprologue@gmail.com) 답변 대기 중. 다음 세션에 답변 확인 + Actions 탭에서 "Supabase Keep-Alive" 수동 실행해 초록불 확인.** ③ 그래도 안 풀리면 **cron-job.org(무료)** 로 매일 Supabase 핑: URL=`https://<프로젝트>.supabase.co/rest/v1/keepalive?select=id&limit=1`, 헤더 `apikey`+`Authorization: Bearer`=anon key. Supabase 7일 방치 시 정지라 며칠 여유 있음.
